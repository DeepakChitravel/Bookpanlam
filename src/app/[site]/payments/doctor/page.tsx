  ...
  useEffect(() => {
    const generateQR = async () => {
      try {
        ...
        // ⭐ SIMPLE: Only CID and AID in UPI note
      ...
      <div className="flex flex-col items-center">
        {loading ? (
          ...
          <>
            ...
            {/* ⭐ Show UPI Note
            <div className="mb-3 p-2 bg-gray-100 rounded-lg w-full text-center">
              <p className="text-xs text-gray-600">UPI Note:</p>
...
export default function PaymentsPage() {
  ...
  const openRazorpay = async () => {
    try {
      ...
      // In the openRazorpay function, update the order creation:
      ...
      const orderResponse = await createRazorpayOrder({
        ...
        // ⭐ NEW: Add service information for JSON storage
        service_reference_type: booking.type === 'department' ? 'department' : 'category',
        category_id: booking.type === 'department' ? booking.departmentId : booking.categoryId,
        service_name: booking.type === 'department'
          ? booking.departmentName
          : (booking.doctorName || booking.categoryName),
      });
      ...
      const options = {
        ...
        // In the openRazorpay function, update the handler section:

        handler: async function (response: any) {
          console.log("🔄 Verifying payment...");

          // Determine service details based on booking type
          const service_type = booking?.type || 'category';
          const service_name = booking?.type === 'department'
            ? booking.departmentName
            : booking.categoryName || booking.doctorSpecialization;
  ...
  const openPayU = async () => {
    try {
      ...
      if (booking?.type === 'department') {
        serviceDisplayName = booking?.departmentName || "Department Service";
      } else {
        serviceDisplayName = booking?.doctorName || booking?.doctor?.doctor_name || booking?.categoryName || "Service";
      }
      ...
      const orderResponse = await createPayUOrder({
        ...
        category_id: booking.categoryId || booking.category_id,
        batch_id: booking.batchId || booking.batch_id,
        // Add service information for JSON
        service_type: booking.type || 'category',
        service_name: serviceDisplayName, // Pass display name
      });
      ...
      if (!orderResponse.success) {
        showNotification(
          "error",
          "PayU Order Failed",
          orderResponse.message || "Failed to create PayU payment order.",
          ["Please try again", "Select another payment method"]
        );
        setIsProcessing(false);
        return;
      ...
      if (orderResponse.service_info) {
        console.log("🟢 PayU Service Info:", orderResponse.service_info);
      }
  ...
  // In your handleCOH function in page.tsx, update the cohData object:
  ...
  const handleCOH = async () => {
    try {
      ...
      const cohData = {
        ...
        category_id: booking.categoryId || booking.category_id,
        payment_method: 'cash',
        // Add department support with service_type
        service_type: booking.type || 'category',
        service_name: serviceDisplayName, // Pass display name
      };

      console.log("🟡 Sending COH data:", cohData);
      ...
      if (data.success) {
        ...
        let serviceInfoDisplay = "Service";
        if (data.service_info && data.service_info.service_name_display) {
          serviceInfoDisplay = data.service_info.service_name_display;
        } else if (data.service_info && data.service_info.service_name_json) {
          try {
            const serviceJson = JSON.parse(data.service_info.service_name_json);
            if (serviceJson.doctor_name) {
              serviceInfoDisplay = serviceJson.doctor_name;
            } else if (serviceJson.department_name) {
              serviceInfoDisplay = serviceJson.department_name;
            } else if (serviceJson.service_name) {
              serviceInfoDisplay = serviceJson.service_name;
            }
          } catch (e) {
        ...
        const updatedBooking = {
          ...booking,
          appointment_id: data.appointment_id,
          service_info: data.service_info // Store service info
        };
        ...
        showNotification(
          ...
          `Your Cash on Hand appointment for ${serviceInfoDisplay} has been booked successfully.`,
          [
  ...
  // Get UPI methods from manual payments
  ...
  useEffect(() => {
    ...
    setBooking(parsed);
    // In your useEffect after setting the booking:
  ...
  // Add this useEffect to filter payments:
  useEffect(() => {
  ...
  // Update the UPI methods check:
  const upiMethods = filteredManualPayments.filter(m => m.upi_id && m.upi_id.trim() !== '');
  const hasUPIMethods = upiMethods.length > 0 && paymentAvailability?.upi_payments?.allowed !== false;
              ...
              <div className="mb-8 p-5 bg-green-50 border border-green-200 rounded-xl">
                ...
                <ol className="space-y-2 text-green-800">
                  ...
                  <li className="flex items-start gap-2">
                    ...
                    <span>Tap on &quot;Scan QR Code&quot; or scan any QR code above</span>
                  </li>
                  <li className="flex items-start gap-2">
